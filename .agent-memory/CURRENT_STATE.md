...
- **PR #37 (Current)**: Sidebar Controlled Drive Filter & Internal Folders Presentation — updated sidebar navigation tree to display exclusively the active selected/controlled drive (`controlledDrive`) under "This PC", removing all unselected drives (e.g. `Local Disk (C:)`), while clearly presenting the organized quadrant folders inside that drive (`Inbox`, `Clients` with client subfolders, `Archive`, and `Review Queue`), and dynamically binding all paths to `${controlledDrive.letter}`.
