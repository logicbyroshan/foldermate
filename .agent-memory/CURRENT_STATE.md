...
- **PR #24 (Current)**: FAQ accordion layout refactored into two independent vertical columns (`.faq-column`) with `align-items: start`, eliminating row-height stretching and preventing unopened cards from expanding into tall empty boxes when adjacent cards open.
