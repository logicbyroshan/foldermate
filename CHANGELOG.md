...
- **PR #47 — Comprehensive Codebase Audit, Dead Code Elimination, Type Hardening & Lint Standardization**:
  ...
  - **Unused Import Cleanup**: Removed 40+ unused imports across 18 desktop views and components (`App.tsx`, `ActivationModal.tsx`, `DriveCustomizerModal.tsx`, `DriveSearchModal.tsx`, `ExplorerHeader.tsx`, `FolderCustomizerModal.tsx`, `InspectorPanel.tsx`, `Sidebar.tsx`, `CommandPalette.tsx`, `FilePreviewCanvas.tsx`, `BackgroundAutomation.tsx`, `ExplorerView.tsx`, `HomeView.tsx`, `KeyboardShortcuts.tsx`, `ReviewQueue.tsx`, `Rules.tsx`, `Search.tsx`, `Settings.tsx`), engine modules (`two-phase-mover.ts`), and tests (`coreldraw-adapter.test.ts`).
  - **Engine Subsystem Type Hardening**: Fixed `VersionEngine` instantiation arguments in `apps/engine/src/index.ts` and `tests/e2e-pipeline.test.ts`; added missing `dispose()` lifecycle method on `CorelDrawAdapter`; resolved `broadcastEvent` invocation and typed payload unpacking in `apps/engine/src/queue/file-pipeline.ts`; supplied `originalName` to `reviewQueue.create` in `apps/engine/src/review/review-manager.ts`; hardened `file-watcher.ts` error handler type; made `category`, `status`, and `createdBy` optional on `ProjectDTO` and `FileVersionDTO` matching DB defaults.
  - **Root Toolchain & Lint Command Standardization**: Replaced missing `eslint` script in root `package.json` with `tsc --noEmit` and configured root `tsconfig.json` exclude for frontend bundler workspaces (`apps/desktop`, `apps/landing`), achieving 0-error typechecking and 100% passing tests (40/40 Vitest tests).
- **PR #45 — Inspector Panel Two-Tab Layout: Preview & Intelligence**:
  - **Tab 1: Preview (Default)**: Shows a mini identity strip (file format icon, extension type, size, version badge), the full `FilePreviewCanvas` visual rendering of the file's content, and pinned Open File / Show in Explorer action buttons — exactly like Windows Explorer's preview pane.
  - **Tab 2: Intelligence**: Shows Classification metadata (Client, Project, Year, Modified), all quick action buttons (Open, Show in Explorer, Create Version), Storage Location path, SHA-256 integrity hash (click-to-copy), and Version History DAG lineage tree.
  - **Folder Tabs**: Preview tab shows a large animated folder icon in an ambient warm gradient card with accent color swatches; Intelligence tab shows project/file/activity stats and color picker.
  - **Smart Header**: Inspector header title now shows the selected file's name + format icon when a file is selected (instead of the generic 'Details & Intelligence' label).
  - **Tab Strip CSS**: `.inspector-tab-strip`, `.inspector-tab-btn` (active state with amber `var(--brand-primary)` underline), `.inspector-tab-content`, `.inspector-preview-identity`, `.inspector-preview-actions`, `.inspector-folder-preview-art`, `.inspector-title-name`.
